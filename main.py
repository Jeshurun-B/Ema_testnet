r"""
====================================================================================================
ALGORITHM: main.py — Always-Awake Daemon with In-Memory Caching & Clock-First Priority
====================================================================================================
Purpose:
  Autonomous 5.5-hour trading engine. Operates on a silent, non-blocking 10-second heartbeat loop
  with in-memory state caching to eliminate Supabase 504 gateway timeouts. Prioritizes the clock
  to fire 15-minute candle pipelines surgically at T+5.0s past the close. Strictly enforces pure
...
Key Architectural Invariants:
  1. In-Memory State Caching (Eliminating the Supabase 504 Storm):
     - Tracks active positions locally in `active_by_symbol`.
     - When flat (0/5 slots active), the 10-second maintenance tick skips Supabase completely.
     - Reduces Supabase REST queries from 360/hour down to 4/hour when idle, preventing PostgREST
       gateway overloads while keeping latency sub-second.
  2. Clock-First Scheduling Priority:
     - The main loop inspects the 15-minute candle boundary FIRST before running maintenance.
     - Guarantees the 15-minute pipeline executes with surgical precision at T+5.0s (:00:05, :15:05,
       :30:05, :45:05), immune to any background maintenance delays.
  3. Pure Event-Driven Crossover Exits:
     - Trades run under the protection of their resting Dynamic TP and Dynamic SL brackets on Binance.
     - A trade terminates early IF AND ONLY IF a completed 15m candle registers a formal opposite
       9/15 EMA crossover (Index [-1] vs [-2]).
     - On reversal: purges resting brackets immediately and executes a Market Close (reduceOnly = True).
  4. Silent Vigilance Mode:
     - Produces zero terminal output during idle 10-second ticks. Emits logs only when actionable
       events occur (fills, bracket deployment, timeouts, closures, or 15-minute evaluations).
  5. Autonomous Self-Chaining at 320 Minutes:
     - Dispatches successor runner via GitHub Actions REST API at 5h 20m for 24/7 continuous operation.

Algorithm Steps:
  Step 1: Module Setup, Raw Docstring Initialization & Line Buffering:
          - Import standard libraries, requests, pandas, ccxt. Force line buffering on stdout.
  Step 2: In-Memory Engine Initialization:
          ...
          - Pre-load all 48 models into RAM once to eliminate disk I/O.
  ...
  Step 4: Smart In-Flight Maintenance Routine (Cache-Gated):
          - If zero slots are active in local memory, exit immediately (zero Supabase traffic).
          - If active trades exist, inspect entry fills, deploy brackets, and check 15m order timeouts.
  Step 5: 15-Minute Pipeline (Clock-First Priority & Pure Crossover Exits):
          - Refresh active trades from Supabase at candle close.
          - Ingest completed multi-timeframe bars (15m, 4h, 1d) via the German proxy tunnel.
          - Detect 9/15 EMA crossover on completed candle [-1] vs [-2].
          - On confirmed opposite crossover, liquidate position immediately via Market Close.
          - On valid signal: extract 25 features, run RAM inference, evaluate R:R >= 2.0 hurdle,
            and route quantized limit entry or log rejection telemetry to Table 2.
  Step 6: Master Execution Loop (Clock-First Priority):
          - Inspects clock on every 10s tick.
          - Prioritizes candle evaluation at T+5.0s before running maintenance.
====================================================================================================
"""
...
# =============================================================================
print("===============================================================================")
print("  EMA_TESTNET PRODUCTION DAEMON (IN-MEMORY CACHING & CLOCK-FIRST PRIORITY)     ")
...
def run_position_maintenance():
    ...
    CACHE ENFORCEMENT: If no trades are active in local memory, skips database polling completely.
    """
    global active_by_symbol
    
    # ── 99% TRAFFIC REDUCTION GUARD ──
    # If no slots are active, exit immediately. Zero Supabase queries during idle periods!
    if not active_by_symbol:
        return

    # If active trades exist, sync latest records from Supabase
...
def run_candle_close_pipeline():
    """
    Evaluates completed 15m candle close.
    ...
    for symbol in ACTIVE_SYMBOLS:
        try:
            ...
            # If NO crossover is registered on this bar, do nothing. Active trades run under resting brackets.
            if not signal:
                continue
...
def main():
    """
    Master daemon loop. Evaluates the clock FIRST to trigger candle pipeline at T+5.0s,
    then executes silent in-flight maintenance.
    """
    ...
    while True:
        try:
            ...
            # 2. CLOCK-FIRST PRIORITY: Detect completed 15m candle at T+5.0s
            ...
            # 3. Silent Cache-Gated Maintenance (Runs only if trades exist locally)
            run_position_maintenance()

            # 4. Silent Sleep for 10 Seconds
